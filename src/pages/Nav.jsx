...
const Nav = () => {
    ...
    const getParentInfo = async () => {
        try {
            ...
            if (user) {
                const snapshot = await db.ref('Bottle').orderByChild('code').equalTo(user).once('value');
                if (snapshot.exists()) {
                    snapshot.forEach(childSnapshot => {
                        setParentInfo(childSnapshot.val());
                    });
                } else {
                    console.log("Parent information not found.");
                }
            } else {
                console.log("User not found in localStorage.");
            }
        } catch (error) {
            console.log("Error fetching parent information: " + error.message);
        }
    };
